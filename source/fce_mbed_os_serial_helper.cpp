...
extern Serial pc;//taken from pal_startup.cpp (mbed-clien-pal) as the Serial object initialized as global.

FtcdCommBase *fce_create_comm_interface(void)
{
    const uint8_t msg_header_token[] = FTCD_MSG_HEADER_TOKEN_FCC;
    return new FtcdCommSerial(&pc, FTCD_COMM_NET_ENDIANNESS_LITTLE, msg_header_token, true);
}
