...
mbed compile \
	-v -c --source .  --source=mbed-cloud-client/mbed-client-pal/Examples/PlatformBSP --artifact-name factory-configurator-client-example --build "BUILD/$PLATFORM/$TOOLCHAIN/$INTERFACE" -t "$TOOLCHAIN" -m "$PLATFORM" --profile="$CFLAGS" -D"$INTERFACE" -DTEST_SERIAL_MULTI_MESSAGE
